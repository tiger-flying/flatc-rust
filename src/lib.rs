...
impl Flatc {
    ...
    fn generate_mod_rs_data_for_dir(&self, dir: &DirEntry) -> Result<String> {
        let module_name = dir.file_name().into_string();

        match module_name {
            Ok(module_name) => {
                // NOTE extra "{" here to escape "{"
                let mut mod_rs_snippet = format!(
                    r#"// @generated
pub mod {} {{
use super::*;
"#,
                            ...
                            if let Some(file_ext) = file_ext {
                                    ...
                                    && (file.file_name() != OsString::from("mod.rs"))
                                {
                                    ...
                                    let file_name_without_ext =
                                        format!("{:?}", file_name_without_ext).replace(r#"""#, "");
                                    mod_rs_snippet.push_str(&format!(
                                        r#"
mod {};
pub use self::{}::*;
"#,
    ...
    pub fn run(&self, args: Args) -> Result<()> {
        ...
        let mut gen_modules_in_separate_dirs = false;
        for extra_arg in args.extra {
            if *extra_arg == "--rust-module-root-file" {
                gen_modules_in_separate_dirs = true;
            }
        ...
        let mut mod_rs_snippets = Vec::<String>::new();
        mod_rs_snippets
            .push("// Automatically generated by flatc-rust. Do not modify.\n".to_string());
        if gen_modules_in_separate_dirs {
